...
namespace LotusLib
{
	...
	class PackageCollection
	{
		...
		std::unordered_map<std::string, Package<T>> m_pkgs;
		Logger& m_log;
	...
	public:
		...
		typename std::unordered_map<std::string, Package<T>>::iterator begin() { return m_pkgs.begin(); }
		typename std::unordered_map<std::string, Package<T>>::iterator end() { return m_pkgs.end(); }
		typename std::unordered_map<std::string, Package<T>>::const_iterator begin() const { return m_pkgs.begin(); }
		typename std::unordered_map<std::string, Package<T>>::const_iterator end() const { return m_pkgs.end(); }

		const Package<T>&
		operator[](const std::string& pkgName) const
		{
			return m_pkgs.at(pkgName);
		}
	...
	protected:
		void _loadPackages()
		{
			for (auto& f : std::filesystem::directory_iterator(m_packageDir))
			{
				...
				if (m_pkgs.find(pkgName) == m_pkgs.end())
		...
		virtual void _loadPackage(std::string pkgName)
		{
			...
			m_pkgs.emplace(
				...
				std::forward_as_tuple(m_packageDir, pkgName, m_isPostEnsmallening)
			);
		} 
	};
}
